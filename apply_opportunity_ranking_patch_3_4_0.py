#!/usr/bin/env python3
"""Install Market Forecaster Multi-Ticker Opportunity Ranking 3.4.0."""
...
PAYLOAD_NAME = "mf34_payload_v340"
...
def backup(path: Path, repo_root: Path):
    if path.exists():
        ...
        target = path.with_name(path.name + f".rankbak_{stamp}")
...
if not payload_root.exists():
    raise SystemExit(f"3.4 payload missing: {payload_root}")

config_path = repo_root / "market_forecaster" / "config.py"
config = config_path.read_text(encoding="utf-8")
if '__version__ = "3.4.0"' not in config and '__version__ = "3.3.0"' not in config:
    raise RuntimeError("3.4 requires Market Forecaster 3.3.0")

for rel in [
    "core/opportunity_ranking.py",
    "ui/ranking_panel.py",
    "api/routes/opportunity_ranking.py",
...
config = config_path.read_text(encoding="utf-8")
if '__version__ = "3.4.0"' not in config:
    config = config.replace('__version__ = "3.3.0"', '__version__ = "3.4.0"', 1)
config_path.write_text(config, encoding="utf-8")
print("patched: market_forecaster/config.py -> 3.4.0")

print("\nMulti-Ticker Opportunity Ranking 3.4.0 installed successfully.")
print("Next:")
print("  python -m compileall -q market_forecaster")
print("  python -m pytest market_forecaster/tests/test_opportunity_ranking.py -q")
